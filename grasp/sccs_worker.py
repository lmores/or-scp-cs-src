...
from utils.misc import current_human_datetime
...
def run_two_stage_sccs(
    ...
    # Init
    init_time_start = time()
    data = read_instance(instance_id, conflict_threshold)
    info = build_instance_info(data)
    init_time = time() - init_time_start

    # Execute first stage
    stage1_result = _solve_sccs(data, info, max_candidates, shared_cache,
        incumbent, None, stage1_time_limit - init_time, iterations_limit, stale_iterations_limit)
    stage1_result.stage = 1
    ...
    lateness = max(0, stage1_result.solve_time + init_time + inter_stage_time - stage1_time_limit)
    stage2_time_start = time()
    stage2_result = _solve_sccs(data, info, max_candidates, shared_cache,
        incumbent, stage2_idxs, stage2_time_limit - lateness, iterations_limit, stale_iterations_limit)
...
def _solve_sccs(
    ...
    while (
        ...
        while time() - solve_time_start < time_limit:
            ...
            if time() - solve_time_start > time_limit:
                break
